# Example: composing workflow.Run with provider, tools, ledger, and memory

This walkthrough composes `workflow.Run` with five packages that `workflow`
...
`workflow.New` run, or the caller-supplied `AckWait` closure `Run` calls
once per gated step. See
[packages/workflow.md](../packages/workflow.md#composing-with-provider-tools-mcp-ledger-and-memory)
...
// into an *workflow.Agent under a freshly generated identity. It also
// returns a second identity, the one the AckWait closure signs the
// confirmed Ack with.
func buildAgent(payload string) (*workflow.Agent, *identity.Identity, error) {
	id, err := identity.New()
	if err != nil {
		return nil, nil, err
	}
...
// buildWait returns the workflow.AckWait closure that runs the review
...
// confirms the ack.
func buildWait(reg *tools.Registry, store *memory.Store, id *identity.Identity, resultRef *string) workflow.AckWait {
...
func runMachine(ctx context.Context, a *workflow.Agent, wait workflow.AckWait, bus *events.Bus) (machine.Status, error) {
...
sequenceDiagram
    ...
    Main->>Main: buildAgent(payload) before flow.New/workflow.New
...
plan-construction time, before `flow.New` and `workflow.New` run, because
...
into an `*workflow.Agent`. It also returns a second identity, the one the
...
`workflow.Run` invocation one level up, is the task body a `ledger` owner
claims and executes.
...
`runMachine` drives `workflow.Run` with its existing nine positional
arguments, over a one-transition machine from `queued` to `reviewed`.
`main` checks `runMachine`'s returned error the same way
[workflow-dispatch.md](workflow-dispatch.md) checks every call: on a
...
// default-tag program in ../_workflowcomposition/main.go.
const (
	ledgerActor = ledger.Actor("composer")
	ledgerOwner = ledger.OwnerID("worker-1")
	ledgerKey   = ledger.IdempotencyKey("task-composition-1")
...
```sh
go run -tags ledger_sqlite ./docs/examples/_workflowcompositionsqlite
```
...
A plain `go run ./docs/examples/_workflowcompositionsqlite`, without the
