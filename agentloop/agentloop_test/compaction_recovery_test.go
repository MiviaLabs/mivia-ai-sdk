...
func newRecoveryFixture(t *testing.T, w contextplan.Window, div int, errs []error, responses []provider.Response, summaryErr error) (*agentloop.Loop, *planningFixture) {
	...
	loop, err := agentloop.New(agentloop.Options{
		Completer: sc,
		Tools:     reg,
		Bounds:    agentloop.Bounds{MaxIterations: 4}, Compaction: agentloop.Compaction{Window: &w, Summarizer: summarizer, Calibrated: contextplan.Calibrate(scaleEstimator{div: div}, 1.0)},
	})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
