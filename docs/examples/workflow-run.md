# Example: running a composed pipeline through workflow/run

This walkthrough shrinks the composition wiring from
[workflow-composition.md](workflow-composition.md) into one
`run.Options` literal. In that example the caller hand-writes an
...
`run.New` does both for you. It validates the option combinations,
...
reads step one's recorded artifact through `run.PayloadOf`, so the
...
```go
// Command workflowrun walks a two-step pipeline through the workflow/run
...
// into one run.Options literal. run.New validates the matrix,
...
// ritual see docs/examples/workflow-composition.md for that older shape.
package main
...
// artifact through run.PayloadOf, so its own tool sees the step
// one result as its payload.
func buildPlan(artifacts *run.Artifacts) (*flow.Definition, error) {
...
func buildAgent(plan *flow.Definition) (*workflow.Agent, error) {
	id, err := identity.New()
	if err != nil {
		return nil, err
	}
	...
	return workflow.New(id, card, plan)
}
...
The `run.Options` literal holds everything the run needs. `New`
