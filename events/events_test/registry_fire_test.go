...
// countingHandler returns a HookHandler that allows and counts its calls
// through calls.
func countingHandler(calls *int) events.HookHandler {
...
func TestFireInvalidPointCallsNoHandler(t *testing.T) {
	...
	calls := 0
	if err := r.Add(events.PointPreTool, "canary", countingHandler(&calls)); err != nil {
		t.Fatalf("Add: %v", err)
	}
...
func TestFireOneAllowingHandlerReturnsNil(t *testing.T) {
	r := events.NewRegistry()
	calls := 0
	if err := r.Add(events.PointPostTool, "only", countingHandler(&calls)); err != nil {
		t.Fatalf("Add: %v", err)
	}
...
func TestFireMiddleVetoStopsChain(t *testing.T) {
	r := events.NewRegistry()
	first, third := 0, 0
	if err := r.Add(events.PointPreTool, "first", countingHandler(&first)); err != nil {
		t.Fatalf("Add(first): %v", err)
	}
	veto := func(context.Context, any) (bool, error) { return false, nil }
	if err := r.Add(events.PointPreTool, "veto", veto); err != nil {
		t.Fatalf("Add(veto): %v", err)
	}
	if err := r.Add(events.PointPreTool, "third", countingHandler(&third)); err != nil {
		t.Fatalf("Add(third): %v", err)
	}

	err := r.Fire(context.Background(), events.PointPreTool, nil)
...
func TestFireRunsHandlersInRegistrationOrder(t *testing.T) {
	r := events.NewRegistry()
	var order []string
	appendHandler := func(name string) events.HookHandler {
		return func(context.Context, any) (bool, error) {
	...
	for _, name := range []string{"alpha", "beta", "gamma"} {
		if err := r.Add(events.PointStop, name, appendHandler(name)); err != nil {
...
func TestFirePassesPayloadToEveryHandler(t *testing.T) {
	...
	for _, name := range []string{"one", "two"} {
		if err := r.Add(events.PointPreTool, name, receiver); err != nil {
			t.Fatalf("Add(%s): %v", name, err)
		}
	}
	if err := r.Fire(context.Background(), events.PointPreTool, payload); err != nil {
		t.Fatalf("Fire = %v, want nil", err)
	}
	if len(seen) != 2 {
...
func TestFireHandlerErrorStopsChain(t *testing.T) {
	...
	if err := r.Add(events.PointPostTool, "first", allowHandler()); err != nil {
		t.Fatalf("Add(first): %v", err)
	}
	if err := r.Add(events.PointPostTool, "failing", failing); err != nil {
		t.Fatalf("Add(failing): %v", err)
	}
	if err := r.Add(events.PointPostTool, "third", countingHandler(&third)); err != nil {
		t.Fatalf("Add(third): %v", err)
	}

	err := r.Fire(context.Background(), events.PointPostTool, nil)
...
func TestFirePointIsolation(t *testing.T) {
	r := events.NewRegistry()
	calls := 0
	if err := r.Add(events.PointPreTool, "pre", countingHandler(&calls)); err != nil {
		t.Fatalf("Add: %v", err)
	}
	if err := r.Fire(context.Background(), events.PointPostTool, nil); err != nil {
