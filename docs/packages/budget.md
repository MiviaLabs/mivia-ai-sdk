# Package reference: context/budget

`context/budget` is a pure, storage-agnostic way to state and check a
...
exported surface below mirrors `api/context/budget.txt`.
...
- `Limits.Validate` fails when `MaxBytes` is negative. Checked first;
  ...
  by `context/budget_test.go`.
- `Limits.Validate` fails when `MaxEvents` is negative and `MaxBytes`
  is not. Pinned by `context/budget_test.go`.

## Cross-references

- [workflow.md](workflow.md) — `workflow.Run` takes an optional
  `*context/budget.Limits` and calls `Validate` once and `Fits` before
...
```go
budget := context/budget.Limits{MaxBytes: 4096, MaxEvents: 20}
