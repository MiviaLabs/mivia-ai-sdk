...
func (l *Loop) run(ctx context.Context, msgs []provider.Message, steer *Steer) (Result, error) {
	...
	for {
		...
		// before the MaxIterations check. A nil or empty return is a
		// no-op; a non-empty return grows history by those messages
		// without counting against MaxIterations (the cap is on
		// Completer calls, not on the number of injected frames). A
		// nil steer is the Run (non-steerable) path, which never
		// installs an injector; skip the call to avoid a nil-deref.
		if steer != nil {
...
func (l *Loop) runToolStage(ctx context.Context, history []provider.Message, resp provider.Response, iterations int, totalUsage provider.Usage, consecutiveFailures *int, noticeInRequest bool, surface runSurface) (Result, bool, error) {
	...
	newHistory, veto, allFailed, err := l.runToolCalls(ctx, history, resp.ToolCalls, iterations, surface)
	if err != nil {
