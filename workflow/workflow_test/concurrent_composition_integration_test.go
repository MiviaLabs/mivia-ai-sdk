...
func newSharedBlocks(t testing.TB) *sharedBlocks {
	...
	for _, name := range []events.Name{
		...
		workflow.ThreadVerifiedEvent, flow.StepCompletedEvent,
	} {
...
func sharedWait(sb *sharedBlocks, refs *sync.Map, index int) workflow.AckWait {
...
// workflow.Run calls at once over one instance of every stateful block.
...
func assertSharedCounters(t *testing.T, sb *sharedBlocks) {
	...
	for _, name := range []events.Name{workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, flow.StepCompletedEvent} {
		if got := sb.counts[name].Load(); got != gated {
			t.Fatalf("%s fired %d times, want %d", name, got, gated)
		}
	}
	if got := sb.counts[workflow.ThreadVerifiedEvent].Load(); got != concurrentRuns {
		t.Fatalf("%s fired %d times, want %d", workflow.ThreadVerifiedEvent, got, concurrentRuns)
	}
