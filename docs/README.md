...
usage, contextplan, contextsession, channel, trigger, trace, skills, scheduler, workflow,
workflow/run, agentloop, runconfig, subagent, taskrun, e2e, envfile,
...
- [packages/a2aack.md](packages/a2aack.md) — the remote step ack: turn a remote A2A task round trip into a `workflow.AckWait` through one send, poll, result, verify, and ack loop.
...
- [packages/runconfig.md](packages/runconfig.md) — the JSON-document loader that binds a step graph to `run.Options`: parse, validate, and wire a flow, its machine, its tools, and its subagents from one config file.
