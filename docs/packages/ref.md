...
`context/ref` provides the canonical content-reference minter and parser for the SDK.
...
Callers like `contextstate`, `envelope`, and `context/plan` rely on `context/ref`.
