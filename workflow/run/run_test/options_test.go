...
func runNewRejections(t *testing.T, valid run.Options, cases []newRejectCase) {
	t.Helper()
	for _, c := range cases {
...
func newRejectSetup(t *testing.T) (ca *captureAsk, scope *tools.Scope, valid run.Options) {
...
func TestNewAccept(t *testing.T) {
	...
	for _, name := range []events.Name{
		workflow.MessageDeliveredEvent,
		workflow.MessageAckedEvent,
...
func TestNewUsesCallerBus(t *testing.T) {
	...
	if err := runner.Bus().Emit(context.Background(), events.Event{Name: workflow.MessageDeliveredEvent, Data: "x"}); err != nil {
...
func TestRunRejectsEmptyThread(t *testing.T) {
	...
	if !errors.Is(err, workflow.ErrNoThread) {
		t.Fatalf("Run error = %v, want workflow.ErrNoThread", err)
	}
}
