...
func TestRenderEqualSummariesRenderEqualText(t *testing.T) {
	a := plan.Summary{Objective: "o", State: "s", Risks: []string{"r1", "r2"}}
	b := plan.Summary{Objective: "o", State: "s", Risks: []string{"r1", "r2"}}
...
func TestSummaryMessage(t *testing.T) {
	s := plan.Summary{Objective: "o", State: "s"}
	msg := plan.SummaryMessage(s)
	if msg.Role != provider.RoleUser {
		t.Fatalf("SummaryMessage role = %q, want %q", msg.Role, provider.RoleUser)
	}
	if msg.Name != plan.SummaryMessageName {
		t.Fatalf("SummaryMessage name = %q, want %q", msg.Name, plan.SummaryMessageName)
	}
	want := plan.SummaryPreamble + "\n" + s.Render()
	if msg.Content != want {
...
func TestSummaryMessagePreamble(t *testing.T) {
	s := plan.Summary{Objective: "o", State: "s"}
	msg := plan.SummaryMessage(s)
	if !strings.HasPrefix(msg.Content, plan.SummaryPreamble) {
		t.Fatalf("SummaryMessage content lacks the preamble prefix: %q", msg.Content)
	}
	rest, ok := strings.CutPrefix(msg.Content, plan.SummaryPreamble)
	if !ok || rest == "" || rest[0] != '\n' {
	...
	if strings.Contains(s.Render(), plan.SummaryPreamble) {
