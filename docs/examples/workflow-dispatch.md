...
func main() {
	...
	for _, name := range []events.Name{workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, workflow.ThreadVerifiedEvent} {
...
`task.dispatch`; `workflow.New` validates the card before it binds
...
`workflow.MessageDeliveredEvent`, `workflow.MessageAckedEvent`, and
`workflow.ThreadVerifiedEvent` before the run starts. The captured order
