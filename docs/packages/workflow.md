...
The workflow package is the composition layer. It binds one identity, one
...
messages. The exported surface below mirrors `api/workflow.txt`.
...
- `ErrNoIdentity` ("workflow: identity is required") — `New` returns it
  when `id` is nil, and `Run` returns it as its first check when the
  receiver `a` or `a.id` is nil. Pinned by
  `workflow_test/definition_test.go`.
- `ErrNoPlan` ("workflow: plan is required") — `New` returns it when
  `plan` is nil. Pinned by `workflow_test/definition_test.go`.
- `ErrNoBus` ("workflow: bus is required") — `EmitMessageDelivered`,
  `EmitMessageAcked`, `EmitThreadVerified`, and `Run` return it when
  `bus` is nil. Pinned by `workflow_test/translator_test.go` and
  `workflow_test/run_test.go`.
- `ErrEscalated` ("workflow: step escalated") — an `AckWait`
  implementation wraps it to route a step to a human instead of
  resolving an ack; `Run` propagates it unchanged. Pinned by
  `workflow_test/run_test.go` and `workflow_test/lifecycle_integration_test.go`.
- `ErrNoWait` ("workflow: wait is required") — `Run` returns it when
  `wait` is nil. Pinned by `workflow_test/run_test.go`.
- `ErrNoThread` ("workflow: thread id is required") — `Run` returns it
  when `threadID` is empty. Pinned by `workflow_test/run_test.go`.
- `ErrOverBudget` ("workflow: context budget exceeded") — `Run`'s
  `confirmStep` wraps it when a gated step's `Fits` check against a
  non-nil `budget` fails. Pinned by `workflow_test/run_budget_test.go`.
...
`workflow` imports none of `provider`, `tools`, `mcp`, `ledger`, or
...
- `mcp`. `mcp.RegisterAll` maps an MCP server's tools onto
  ...
  `Run` starts; `workflow` never needs to import `mcp`.
- `ledger`. A caller calls `ledger.Claim`, runs `workflow.Run` as the
...
See [../examples/workflow-composition.md](../examples/workflow-composition.md)
...
_ = bus.Subscribe(workflow.MessageDeliveredEvent, noop)
_ = bus.Subscribe(workflow.MessageAckedEvent, noop)
_ = bus.Subscribe(workflow.ThreadVerifiedEvent, noop)
...
if err != nil {
    // a step failed, escalated (errors.Is(err, workflow.ErrEscalated)),
