...
Phases 78, 79, 80, 81, 82, and 84 are plan-only and not scheduled. A
gap analysis compared `agentloop` against `internal/agent.Loop`, a
production caller in a separate, external repository
(`mivia-agent`), and found seven capabilities that caller needs and
`agentloop` lacks. Each gap is its own phase: steering and
interruption (78), graceful work-limit conclude (79), per-batch
tool-result size shaping (80), duplicate-call dedup within a turn
(81), prompt-injection-safe hook and steer framing (82), heartbeat
