#!/usr/bin/env python3
"""Gate: every package needs a real internal caller, or a declared
exemption. An orphan is a package no other package's non-test .go file
imports. A package at any depth counts, both as an orphan and as a
caller. Ground truth is the resolved import graph, not
policy/layers.json, which states permission, not use. An orphan missing
from policy/pending_wiring.json fails; so does a pending_wiring.json
entry that is stale: its package no longer exists, or it has gained a
real caller since the entry was written."""
import argparse
import json
import sys
import tempfile
...
def compute_importers(pkgs: dict[str, set[str]]) -> dict[str, set[str]]:
    """compute_importers maps each package path to the set of other
    packages whose non-test .go files import it."""
    importers: dict[str, set[str]] = {p: set() for p in pkgs}
    for pkg, imports in pkgs.items():
        for imp in imports:
...
def find_orphans(root: Path, env_extra: dict | None = None) -> set[str]:
    """find_orphans returns the package paths with zero non-test internal
    callers."""
    pkgs = go_packages.packages(root, env_extra)
    importers = compute_importers(pkgs)
...
def validate_target(name: str, target: str, pkgs: list[str]) -> list[str]:
    ...
    package. A free-form (multi-word) target is prose about an external
    or test-only consumer, and is not checked."""
    token = target.strip()
    if " " in token:
        return []
    ...
    if token not in pkgs:
        return [f"{name}: target '{token}' names no package directory"]
    return []
...
def check(root: Path, env_extra: dict | None = None) -> list[str]:
    ...
    problems: list[str] = []
    pkgs = go_packages.package_paths(root, env_extra)
    orphans = find_orphans(root, env_extra)
...
def _write_module(root: Path) -> None:
    """_write_module writes the fixture go.mod."""
    go_packages.write_file(root, "go.mod", f"module {go_packages.MODULE}\n\ngo 1.25.0\n")


def _write_pkg(root: Path, path: str, imports: list[str], test_only_import: str | None = None) -> None:
    """_write_pkg writes a minimal fixture package at root/path."""
    name = path.rsplit("/", 1)[-1]
    body = "".join(f'\nimport "{go_packages.MODULE}/{imp}"\n' for imp in imports)
    uses = "".join(f"\nvar _ = {imp.rsplit('/', 1)[-1]}.Sym\n" for imp in imports)
    go_packages.write_file(root, f"{path}/{name}.go", f"package {name}\n{body}\nvar Sym = 1\n{uses}")
    if test_only_import:
        used = test_only_import.rsplit("/", 1)[-1]
        go_packages.write_file(root, f"{path}/{name}_test.go", (
            f'package {name}\n\nimport "{go_packages.MODULE}/{test_only_import}"\n\n'
            f"var _ = {used}.Sym\n"
        ))
...
def _probe_caller_found(root: Path) -> list[str]:
    ...
    if "leaf" in find_orphans(root, _env()):
...
def _probe_test_only_caller_ignored(root: Path) -> list[str]:
    ...
    if "leaf2" not in find_orphans(root, _env()):
        return ["probe_test_only_caller_ignored: a test-only import must not count as a real caller"]
    return []


def _probe_nested_orphan(root: Path) -> list[str]:
    _write_pkg(root, "flow/engine", [])
    _write_pending(root, {})
    if "flow/engine" not in find_orphans(root, _env()):
        return ["probe_nested_orphan: expected 'flow/engine' to be reported orphan"]
    return []


def _probe_nested_caller_counts(root: Path) -> list[str]:
    _write_pkg(root, "leaf3", [])
    _write_pkg(root, "flow/engine", ["leaf3"])
    if "leaf3" in find_orphans(root, _env()):
        return ["probe_nested_caller_counts: a nested importer must count as a real caller"]
    return []
...
def _probe_stale_entry_fails(root: Path) -> list[str]:
    ...
    root_a = root / "a"
    root_a.mkdir()
    _write_module(root_a)
    _write_pkg(root_a, "present", [])
    _write_pending(root_a, {
        "ghost": {"reason": "r", "target": "not yet planned", "permanent": False},
        "present": {"reason": "r", "target": "not yet planned", "permanent": False},
    })
    problems = check(root_a, _env())
...
def run_probe() -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="orphan-probe-") as tmp:
        for fn in (
            ...
            _probe_nested_orphan,
            _probe_nested_caller_counts,
