...
var (
	...
	ErrNoThread   = errors.New("workflow: thread id is required")
	ErrOverBudget = errors.New("workflow: context budget exceeded")
)
	...
	return func(ctx context.Context, step flow.Step) error {
		...
		if budget != nil {
			...
			if !budget.Fits(candidateBytes, stepCount) {
				return fmt.Errorf("workflow: step %q: %w", step.ID, ErrOverBudget)
			}
		}
		if hb != nil {
		...
		if ack.Status != envelope.AckConfirmed {
			return fmt.Errorf("workflow: step %q ack status %q, want %q", step.ID, ack.Status, envelope.AckConfirmed)
		}
