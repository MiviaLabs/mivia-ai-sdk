...
func compactionWindow(b, triggerPercent, targetTokens, recentTail int) plan.Window {
	return plan.Window{
		MaxTokens:  b,
		Reserve:    0,
		Compaction: plan.Compaction{TriggerPercent: triggerPercent, TargetTokens: targetTokens, RecentTail: recentTail},
	}
}
...
func TestCompactBelowTriggerPassesThrough(t *testing.T) {
	...
	res, err := plan.Compact(msgs, plan.Window{MaxTokens: 1000}, byteEstimator{})
	if err != nil {
...
func TestCompactAtTriggerKeepsMandatorySet(t *testing.T) {
	...
	res, err := plan.Compact(msgs, compactionWindow(100, 100, 10, 0), byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
	if !res.Compacted {
