...
func TestRunTwoStepsWithTools(t *testing.T) {
	...
	plan := mustFlow(t, []flow.Step{
		...
		{ID: "ship", To: "shipped", Needs: []string{"review"}, PayloadFrom: run.PayloadOf("review", artifacts)},
	}, nil)
	reg := tools.New()
	addTools(t, reg,
	...
	runner.Bus().Subscribe(workflow.ThreadVerifiedEvent, counter.handler())
	...
	if got := counter.count(workflow.MessageAckedEvent); got != 2 {
		t.Errorf("acked events = %d, want 2", got)
	}
	if got := counter.count(workflow.ThreadVerifiedEvent); got != 1 {
...
func TestRunNonTextResult(t *testing.T) {
	...
	if !errors.Is(err, run.ErrResultNotText) {
...
func TestRunWithMonitor(t *testing.T) {
	...
	// workflow.Run forgets the beaten id on every return path, so the
...
func TestRunResumesArtifactsAcrossProcesses(t *testing.T) {
	...
	firstRunner, err := run.New(run.Options{
	...
	restored, err := run.DecodeArtifacts(data)
	if err != nil {
