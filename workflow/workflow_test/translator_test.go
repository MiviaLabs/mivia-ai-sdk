// Package workflow_test also holds the red-green unit cases for the
...
func TestEmitMessageDeliveredSuccess(t *testing.T) {
	bus := events.New()
	r := &recorder{}
	if err := bus.Subscribe(workflow.MessageDeliveredEvent, r.handler); err != nil {
	...
	name, data := r.last()
	if name != workflow.MessageDeliveredEvent {
		t.Fatalf("event name = %q, want %q", name, workflow.MessageDeliveredEvent)
	}
...
func TestEmitMessageDeliveredBadSignature(t *testing.T) {
	...
	err := workflow.EmitMessageDelivered(context.Background(), bus, m)
	if err == nil {
...
func TestEmitMessageAckedSuccess(t *testing.T) {
	bus := events.New()
	r := &recorder{}
	if err := bus.Subscribe(workflow.MessageAckedEvent, r.handler); err != nil {
		t.Fatalf("Subscribe() unexpected error: %v", err)
	}
	a := validAck(t)
	...
	name, data := r.last()
	if name != workflow.MessageAckedEvent {
		t.Fatalf("event name = %q, want %q", name, workflow.MessageAckedEvent)
	}
...
func TestEmitMessageAckedBlankMessageID(t *testing.T) {
	...
	err := workflow.EmitMessageAcked(context.Background(), bus, a)
	if err == nil {
...
func TestEmitThreadVerifiedSuccess(t *testing.T) {
	bus := events.New()
	r := &recorder{}
	if err := bus.Subscribe(workflow.ThreadVerifiedEvent, r.handler); err != nil {
		t.Fatalf("Subscribe() unexpected error: %v", err)
	}
	msgs := validThread()
	...
	name, data := r.last()
	if name != workflow.ThreadVerifiedEvent {
		t.Fatalf("event name = %q, want %q", name, workflow.ThreadVerifiedEvent)
	}
...
func TestEmitThreadVerifiedBrokenChain(t *testing.T) {
	...
	err := workflow.EmitThreadVerified(context.Background(), bus, msgs)
	if err == nil {
...
func TestEmitNilBusReturnsErrNoBus(t *testing.T) {
	cases := []struct {
		...
		{"EmitMessageDelivered", func() error {
			return workflow.EmitMessageDelivered(context.Background(), nil, signedMessage(t, "msg-nil-bus"))
		}},
		{"EmitMessageAcked", func() error {
			return workflow.EmitMessageAcked(context.Background(), nil, validAck(t))
		}},
		{"EmitThreadVerified", func() error {
			return workflow.EmitThreadVerified(context.Background(), nil, validThread())
		}},
		{"EmitMessageDeliveredInvalidEnvelope", func() error {
			return workflow.EmitMessageDelivered(context.Background(), nil, badSignatureMessage(t))
		}},
		{"EmitMessageAckedInvalidEnvelope", func() error {
			return workflow.EmitMessageAcked(context.Background(), nil, blankMessageIDAck())
		}},
		{"EmitThreadVerifiedInvalidEnvelope", func() error {
			return workflow.EmitThreadVerified(context.Background(), nil, brokenThread())
		}},
	}
...
func TestEmitNoSubscriberReturnsNil(t *testing.T) {
	cases := []struct {
		...
		{"EmitMessageDelivered", func(bus *events.Bus) error {
			return workflow.EmitMessageDelivered(context.Background(), bus, signedMessage(t, "msg-no-sub"))
		}},
		{"EmitMessageAcked", func(bus *events.Bus) error {
			return workflow.EmitMessageAcked(context.Background(), bus, validAck(t))
		}},
		{"EmitThreadVerified", func(bus *events.Bus) error {
			return workflow.EmitThreadVerified(context.Background(), bus, validThread())
		}},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
