// Package workflow_test also holds the run-loop integration cases: a
...
func twoStepFixture(t testing.TB) (*workflow.Agent, *machine.Definition) {
	t.Helper()
...
func TestRunTwoStepOrderAndEvents(t *testing.T) {
	...
	for _, name := range []events.Name{workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, workflow.ThreadVerifiedEvent, flow.StepCompletedEvent} {
...
// workflow.ErrEscalated), the bus receives no ThreadVerifiedEvent, and
...
func TestRunTwoStepEscalatedSecondStep(t *testing.T) {
	...
	wait := func(ctx context.Context, msg envelope.Message) (envelope.Ack, error) {
		calls++
		if msg.ID == "b" {
			return envelope.Ack{}, fmt.Errorf("step %s needs a human: %w", msg.ID, workflow.ErrEscalated)
		}
		ack, err := envelope.NewAck(msg, "receiver", "restating "+msg.ID)
		if err != nil {
			return envelope.Ack{}, err
