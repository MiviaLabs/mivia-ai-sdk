...
func TestMinterAllocationBudget(t *testing.T) {
	data := make([]byte, 4096)
	if n := testing.AllocsPerRun(100, func() { _ = ref.Mint(data) }); n > mintAllocBudget4096 {
		t.Fatalf("Mint of 4096 bytes allocated %v, budget %d", n, mintAllocBudget4096)
	}
	if n := testing.AllocsPerRun(100, func() { _ = ref.Digest(data) }); n > digestAllocBudget4096 {
