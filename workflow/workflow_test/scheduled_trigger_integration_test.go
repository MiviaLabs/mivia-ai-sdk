// Package workflow_test also holds the scheduled and triggered
// invocation test. It proves scheduler.Job and trigger.Action each
// wrap workflow.Run as a plain closure, with ledger admission around the
...
func newInvokedFixture(t testing.TB) *invokedFixture {
	...
	if fx.a, err = workflow.New(id, card, plan); err != nil {
...
// ackWaitFromNotifier adapts a channel.Notifier to workflow.AckWait's
...
// a workflow.AckWait compiles only if the two shapes compose.
func ackWaitFromNotifier(n channel.Notifier, from string) workflow.AckWait {
...
// wraps workflow.Run the same way scheduler.Job does, and that a false
