...
Status: plan. Builds on the shipped mutation kit (phase 54,
...
- New `scripts/mutation_denylist/<pkg>.json` floor files for:
  `workspace`, `subagent`, `agentloop`, `mcp`, `dispatch`,
  `a2aclient`, and `schema`.
- A re-measured floor for `ledger`, reflecting the admission-closure
  fixes landed after its phase 54 floor was set (see item 4 under
  Priority order below). The new floor never drops below the
  current 91.
- A `make mutation-gate` target that loops over every package holding
...
- A one-line addition to each touched package's own plan
  (`docs/plans/workspace.md`, `docs/plans/subagent.md`,
...
commit `6319ae6`, on 2026-08-20. The floor is the measured rate
...
6. `dispatch` — the NDJSON envelope endpoint. Its receive ladder
   ...
   caller-untrusted POST bodies before any handler runs. Measured:
   30 killed, 1 survived, rate 96.77%. Floor: **96**.
7. `a2aclient` — the a2a-go client adapter. Re-verifies the message
   signature after every remote hop over a real gRPC transport.
   Measured: 34 killed, 1 survived, rate 97.14%. Floor: **97**.
...
- **A new `make mutation-gate` target, manual cadence for now**
  ...
  standing cadence (weekly, or before a release) until CI exists.
  Reason: matches AGENTS.md's statement that gates on the committed
  tree stay aspirational until CI exists, and gives the sweep a name
  a future CI workflow can call directly with no rework.
- A CI-only step. Rejected for now: no CI exists in this repo yet
  (a separate, untracked `docs/plans/ci.md` is in progress). This
  plan does not depend on that work landing first; `make
  mutation-gate` works standalone today and slots into CI later with
  no change.
...
seven new files and one updated value (`ledger.json`); neither is a
Go API surface, so `api/` locks are unaffected.
