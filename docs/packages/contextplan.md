...
`contextplan` manages token budget windows, token calibration, and history compaction. It compacts a conversation history down to a target token budget while preserving critical context. The exported surface below mirrors `api/contextplan.txt`.
...
- Compaction keys are deterministic and derived via `contextref.Mint`.

## Cross-references

- [contextref.md](contextref.md) — canonical content address hashing.
