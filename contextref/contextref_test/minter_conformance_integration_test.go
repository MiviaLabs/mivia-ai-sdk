...
func TestEnvelopeContextRefDelegatesToMinter(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			want := contextref.HashPrefix + tc.digest
			if got := contextref.Mint([]byte(tc.content)); got != want {
	...
	t.Run("multi-chunk split", func(t *testing.T) {
		want := contextref.HashPrefix + pinnedWholeDigest
		if got := contextref.Mint([]byte("alpha"), []byte("-omega")); got != want {
...
func TestMemoryPutDelegatesToMinter(t *testing.T) {
	...
	want := contextref.HashPrefix + pinnedTextDigest
	...
	if ref != contextref.Mint([]byte(pinnedText)) {
	...
	if emptyRef != contextref.HashPrefix+pinnedEmptyDigest {
