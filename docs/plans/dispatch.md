...
### Gap fix: ReplayCapacity states the bound it enforces

Status: planned, not yet built. `Options.ReplayCapacity`
(`dispatch/options.go:51`) says it "caps the entry count of the
ledger New builds internally". `ledger.MemStore` did not enforce that
cap. See the "Gap fix: MaxEntries bounds the stored entry count"
section in `docs/plans/ledger.md` for the store-side fix. This
section carries the `dispatch`-side doc correction and one test.

`Endpoint` is an HTTP surface and `replayKey`
(`dispatch/ladder.go:20`) mints one key per sender-supplied
`(ThreadID, ID)` pair. So the store bound is a remote-input bound,
not an internal detail.

#### Doc corrections

Rewrite the `Options.ReplayCapacity` doc to state the enforced bound.
`ReplayCapacity` caps the internal ledger's live records and its
tombstones separately. The store therefore holds at most twice
`ReplayCapacity` records, plus one record per in-flight line.

Add the window sentence to `ReplayCapacity` and to
`DefaultReplayCapacity`. Replay protection covers the most recent
keys, not every key the endpoint ever saw. A key evicted past the
tombstone cap is admitted again and its handler runs again. Size
`ReplayCapacity` above the expected duplicate window.

Correct the mutation-floor paragraph at the end of this plan. It
claims `MemStore` eviction enforces idempotency regardless of
capacity size. That claim stops being true with the store-side fix.
State instead that the two capacity configurations differ only past
the cap, which the new test below now observes.

Update the matching `Options` and capacity entries in
`docs/packages/dispatch.md` in the same change.

#### Test

`TestEndpointReplayLedgerStaysBounded` in `dispatch/dispatch_test/`.
Build `Options.Ledger` over
`ledger.NewMemStoreWithOptions(ledger.MemStoreOptions{MaxEntries: 2})`.
Post 20 distinct signed messages through `Endpoint.Handler`. Count
the store's records through `ledger.MemStore.Range`. Assert the count
is at most 4. Today the count is 20, so the case fails now. The case
proves the remote-controlled key space cannot grow the store without
a bound.

#### Verification for this gap fix

No exported symbol changes in `dispatch`. `make api-update` must
produce no diff for `api/dispatch.txt`. `policy/layers.json` is
unchanged: the `dispatch` row already lists `ledger`. Run `make
verify` and `go test -race ./dispatch/...`. Hold `dispatch` coverage
at or above 85 and its stored mutation floor unchanged.
