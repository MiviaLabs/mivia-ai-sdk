...
func TestExcerptsTruncatePerItem(t *testing.T) {
	...
	long := strings.Repeat("x", plan.MaxFieldBytes+200)
	...
	if len(excerpt) > plan.MaxFieldBytes+1 {
...
func TestExcerptsTruncateInTotal(t *testing.T) {
	...
	for i := range msgs {
		msgs[i] = provider.Message{
			...
			Content: strings.Repeat("y", plan.MaxFieldBytes),
		}
	}
	if _, err := s.Summarize(context.Background(), msgs); err != nil {
		t.Fatalf("Summarize() = %v, want nil", err)
	...
	if len(excerpts) > plan.MaxExcerptTotalBytes {
	...
	if promptTotal > len(req.Messages[0].Content)+plan.MaxExcerptTotalBytes {
...
func TestExcerptsStopAtFirstThatDoesNotFit(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		msgs = append(msgs, provider.Message{
			Role:    provider.RoleUser,
			Content: strings.Repeat("a", plan.MaxFieldBytes),
		})
	}
	if _, err := s.Summarize(context.Background(), msgs); err != nil {
		t.Fatalf("Summarize() = %v, want nil", err)
