...
docs/plans/agents/phase23_flow_fallback.md. Phase 25 shipped the
checkpoint, the pause rule, and `Resume`. Phase 30 shipped a retry
loop; see docs/plans/agents/phase30_flow_retry.md.
...
Shipped in phase 23: failure routing. A fallback step admits on a
failed need and receives the failure context through `FailureFrom`.
...
- `type Admission int` with `AdmissionOnFinished` as the zero-value
  ...
  shipped in phase 23, for the fallback path.
...
Phase 23's tests, shipped, cover the fallback: a handled failure lets
the run complete, the fallback reads the failure context, and an
unhandled failure still aborts.
...
`Step` fields. Phase 23 extended it with `Failure`, `FailureFrom`, the
`AdmissionOnFailed` constant, and the `Failed` field on `Checkpoint`.
Phase 25 extended it with `Checkpoint`, its `Validate`, `Encode`, and
`Decode`, `Resume`, and the changed seven-argument `Run` signature.
Phase 22, phase 23, and phase 25 each left `api/machine.txt` and
