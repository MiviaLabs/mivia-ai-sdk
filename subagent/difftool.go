...
// inside ws, relative to ws's bound root, against proposed content,
// through diff.Unified. A path that does not yet exist diffs against
// empty content. maxLines passes straight to diff.Unified. Not
// privileged: it reads two pieces of content and writes nothing.
func DiffTool(name string, ws *workspace.Workspace, maxLines int) tools.Tool {
	return &diffTool{name: name, ws: ws, maxLines: maxLines}
}
...
// error, including workspace.ErrEscape, propagates unchanged. A diff
// over t.maxLines returns diff.ErrTooLarge unchanged.
func (t *diffTool) Run(ctx context.Context, in tools.InOut) (tools.Out, error) {
	...
	current, err := t.ws.ReadFile(args.Path)
	if err != nil {
