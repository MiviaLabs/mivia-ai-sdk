...
func TestCalibratedEWMAConverges(t *testing.T) {
	c := plan.Calibrate(fixedEstimator{tokens: 100}, 0.5)
	for i := 0; i < 20; i++ {
...
func TestCalibratedBoundsClamp(t *testing.T) {
	c := plan.Calibrate(fixedEstimator{tokens: 10}, 1.0)
	got, err := c.EstimateTokens(provider.Request{})
	if err != nil {
		t.Fatalf("EstimateTokens: %v", err)
	}
	...
	if float64(got2) > 10*plan.MaxCorrectionFactor+1 {
		t.Fatalf("EstimateTokens = %d, correction factor exceeded MaxCorrectionFactor", got2)
	}

	c2 := plan.Calibrate(fixedEstimator{tokens: 10}, 1.0)
	...
	if float64(got2c2) < 10*plan.MinCorrectionFactor-1 {
...
func TestCalibratedAlphaSelection(t *testing.T) {
	for _, alpha := range []float64{0, -1, -0.5, 1.5, 2} {
		c := plan.Calibrate(fixedEstimator{tokens: 100}, alpha)
	...
	slow := plan.Calibrate(fixedEstimator{tokens: 100}, 0.05)
	fast := plan.Calibrate(fixedEstimator{tokens: 100}, 0.9)
...
func TestCalibratedAlphaAboveOneFallsBackToDefault(t *testing.T) {
	withDefault := plan.Calibrate(fixedEstimator{tokens: 100}, plan.DefaultSmoothingFactor)
	aboveOne := plan.Calibrate(fixedEstimator{tokens: 100}, 1.5)
...
func TestCalibratedZeroEstimatesSkip(t *testing.T) {
	c := plan.Calibrate(fixedEstimator{tokens: 0}, 0.5)
	got, err := c.EstimateTokens(provider.Request{})
	if err != nil {
		t.Fatalf("EstimateTokens: %v", err)
	}
...
func TestCalibratedConvergesTowardActualNotGeometricMean(t *testing.T) {
	const raw = 100
	const actual = 150
	c := plan.Calibrate(fixedEstimator{tokens: raw}, 0.5)
...
func TestCalibratedObserveOutOfEstimateOrder(t *testing.T) {
	c := plan.Calibrate(fixedEstimator{tokens: 100}, 0.4)
...
func TestCalibratedEstimateTokensPropagatesError(t *testing.T) {
	...
	c := plan.Calibrate(byteEstimator{err: wantErr}, 0.5)
...
func TestCalibratedConcurrentUse(t *testing.T) {
	...
	if got < int(100*plan.MinCorrectionFactor) || got > int(100*plan.MaxCorrectionFactor) {
...
func TestCalibrateNaNAlphaFallsBack(t *testing.T) {
	...
	c := plan.Calibrate(fixedEstimator{tokens: 10}, nan)
