...
func TestRunEntryChecks(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			status, rec, err := a.Run(context.Background(), tt.threadID, nil, in, tt.wait, tt.bus)
...
func TestRunOneStepConfirmed(t *testing.T) {
	...
	bus := newRunBus(t)
	status, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, confirmingWait, bus)
	if err != nil {
...
func TestRunOneStepCorrected(t *testing.T) {
	a, m := oneStepFixture(t)
	bus := newRunBus(t)
	_, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, correctingWait, bus)
	if err == nil {
...
func TestRunOneStepEscalated(t *testing.T) {
	...
	_, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, escalate, bus)
...
func TestRunOneStepPlainWaitError(t *testing.T) {
	...
	_, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, plainErr, bus)
...
func TestRunConfirmStepMessageAckedNoSubscriber(t *testing.T) {
	...
	_, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, confirmingWait, bus)
	if err == nil {
...
func TestRunZeroStepPlan(t *testing.T) {
	...
	status, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, wait, bus)
	if err != nil {
		t.Fatalf("Run() unexpected error: %v", err)
	}
