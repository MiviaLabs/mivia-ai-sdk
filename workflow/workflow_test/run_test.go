// Package workflow_test also holds the red-green unit cases for Run:
...
func oneStepFixture(t testing.TB) (*workflow.Agent, *machine.Definition) {
	t.Helper()
...
func TestRunZeroValueAgentReturnsErrNoIdentity(t *testing.T) {
	cases := []struct {
		...
		{"nil agent", nil},
		{"zero-value agent", &workflow.Agent{}},
	}
...
func TestRunOneStepEscalated(t *testing.T) {
	...
	escalate := func(ctx context.Context, msg envelope.Message) (envelope.Ack, error) {
		return envelope.Ack{}, fmt.Errorf("step needs a human: %w", workflow.ErrEscalated)
	}
...
func TestRunOneStepWaitErrorWithValidAck(t *testing.T) {
	...
	if err := bus.Subscribe(workflow.MessageDeliveredEvent, func(ctx context.Context, e events.Event) error { return nil }); err != nil {
