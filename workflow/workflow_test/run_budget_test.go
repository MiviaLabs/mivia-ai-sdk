// Package workflow_test also holds the context-budget cases for Run: a
...
func TestRunMaxEventsExceededMidPlan(t *testing.T) {
	...
	if err := bus.Subscribe(workflow.MessageDeliveredEvent, func(ctx context.Context, e events.Event) error {
		delivered++
		return nil
	}); err != nil {
...
func TestRunBudgetFitsFailureNeverBeats(t *testing.T) {
	...
	if !errors.Is(runErr, workflow.ErrOverBudget) {
