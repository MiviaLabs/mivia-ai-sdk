...
func messagingSubRunner(t *testing.T, subID *envelope.Identity, subBox, parentBox *subagent.Mailbox) (*run.Runner, *run.Artifacts) {
	t.Helper()
	...
	a, err := workflow.New(subID, flow.Card{Name: "worker", Capabilities: []string{"c"}}, plan)
	if err != nil {
...
func messagingOrchestrator(t *testing.T, parentID *envelope.Identity, subID *envelope.Identity, subRunner *run.Runner, subArtifacts *run.Artifacts, subBox, parentBox *subagent.Mailbox, r *room.Room) (*run.Runner, *run.Artifacts) {
	t.Helper()
	...
	artifacts := &run.Artifacts{}
	a, err := workflow.New(parentID, flow.Card{Name: "orchestrator", Capabilities: []string{"c"}}, plan)
	if err != nil {
...
func assertMessagingResults(t *testing.T, subArtifacts, orchArtifacts *run.Artifacts, r *room.Room, subID *envelope.Identity) {
	t.Helper()
