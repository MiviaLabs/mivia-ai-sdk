...
- `flow/` — the step graph, the sequential runner, and the parallel
  ...
  `Roots`, `Run`, `Confirm`, `Admission`, `Route`, `Outcome`,
  `Report`, `Failure`, and `FailureFrom`. `Step` carries an optional
  `Sub *Definition` for chaining, an `Admission` rule in `When`, and
  an optional `Route` that makes it a branch step. `Run` walks the
  ...
  admitted through a need that ended `OutcomeFailed`
  ...
  every resolved step's `Outcome`: `OutcomeSucceeded`,
  `OutcomeFailed`, or `OutcomeSkipped`. See
  [packages/flow.md](packages/flow.md).
