...
func TestCompactLatestCompleteAssistantUnitSurvivesWhole(t *testing.T) {
	...
	res, err := plan.Compact(msgs, compactionWindow(1000, 10, 5, 0), byteEstimator{})
	if err != nil {
...
func TestCompactIncompleteAssistantUnitNotSelected(t *testing.T) {
	...
	res, err := plan.Compact(msgs, compactionWindow(100, 10, 5, 0), byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
...
func TestCompactMismatchingReplyIsOwnUnit(t *testing.T) {
	...
	w := plan.Window{MaxTokens: 100, Compaction: plan.Compaction{
		TriggerPercent: 10, TargetPercent: 5, PreserveNames: []string{"keep"},
	}}
	res, err := plan.Compact(msgs, w, byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
	kept := contentSet(res.Kept)
...
func TestCompactNoObjective(t *testing.T) {
	...
	_, err := plan.Compact(msgs, plan.Window{MaxTokens: 100000}, byteEstimator{})
	if !errors.Is(err, plan.ErrNoObjective) {
...
func TestCompactFailures(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			wantErr: plan.ErrEstimateFailed,
		},
		{
			name:    "invalid window",
			msgs:    []provider.Message{{Role: provider.RoleUser, Content: "hi"}},
			w:       plan.Window{MaxTokens: 0},
			e:       byteEstimator{},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			res, err := plan.Compact(c.msgs, c.w, c.e)
...
func TestCompactRetentionOverflow(t *testing.T) {
	...
	_, err := plan.Compact(msgs, plan.Window{MaxTokens: 30}, byteEstimator{})
...
func TestCompactIdempotency(t *testing.T) {
	...
	first, err := plan.Compact(build(`{"q":"x"}`), w, byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
	second, err := plan.Compact(build(`{"q":"x"}`), w, byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
	...
	third, err := plan.Compact(build(`{"q":"y"}`), w, byteEstimator{})
	if err != nil {
		t.Fatalf("Compact() = %v, want nil", err)
	}
	...
	if !strings.HasPrefix(third.Key, plan.CompactionAlgorithm+":") {
		t.Fatalf("key %q lacks the %q prefix", third.Key, plan.CompactionAlgorithm)
	}
}
