// Package workflow_test also holds the cross-cutting heartbeat
...
func TestLivenessTwoConcurrentThreadsShareOneMonitor(t *testing.T) {
	...
	for i, threadID := range threads {
		...
		go func(i int, threadID string) {
			defer wg.Done()
			_, _, err := a.Run(context.Background(), threadID, m, machine.InOut{}, confirmingWait, bus, hb, "", nil)
			errs[i] = err
		}(i, threadID)
	}
	wg.Wait()
...
func TestLivenessExternalSweepCancelsStalledWait(t *testing.T) {
	...
	go func() {
		defer close(done)
		_, _, runErr = a.Run(ctx, "thread-1", m, machine.InOut{}, blockedWait, bus, hb, "", nil)
	}()
...
func TestLivenessPanelWaveReachesNoBeat(t *testing.T) {
	...
	status, _, err := a.Run(context.Background(), "thread-1", m, machine.InOut{}, confirmingWait, bus, hb, "", nil)
	if err != nil {
		t.Fatalf("Run() unexpected error: %v", err)
	}
