...
`lastEst` field happens to hold at the moment `Observe` runs. Today
`Calibrated` holds `lastEst`/`hasLast` as single shared fields, one
mutex guarded, so no data race in the Go race-detector sense, but a
logic race: `EstimateTokens` overwrites `lastEst` on every call, and
`Observe(actual int)` implicitly pairs `actual` against whatever
`lastEst` holds when it runs, not against the estimate that produced
the specific request the caller now reports on. Two real callers
break the single-estimate-then-single-observe assumption this
implicit pairing needs: `contextplan.Planner.Plan` and `Compact` each
call `EstimateTokens` many times per call, for trial insertions and
...
- `Observe`'s internal formula changes from divisive to multiplicative,
  ...
  matching the pre-fix intent, because at the first call `factor` is
  `1.0`, so `estimated == raw` and the first step is byte-for-byte the
  old formula's first step; later calls generalize correctly with no
  raw-tracking needed, since a scaled estimate that already tracks
  `actual` (`sample ≈ 1`) leaves `factor` unchanged.
...
- Any change to the EWMA math, the clamp bounds, or `Calibrate`'s
...
- Any caching of a `(estimate, request)` pairing inside `Calibrated`
  ...
  with an explicit one the package tracks. The caller now owns
  pairing its own estimate to its own actual, which is the only place
...
entry. Every caller of `Observe` needs a matching update; `agentloop`
is the only internal caller today (`agentloop/run.go`).

### Fix companion change: `agentloop`
...
- Inside `runChat`, when `l.calibrated != nil`, call
  ...
  store the result on `chatAttempt.estimatedTokens`, before the
  `Chat` call. A non-nil `l.window` already requires a non-nil
  `l.calibrated` through `Options.Validate`'s `ErrEstimatorRequired`
  rule, so this call is safe to gate on `l.calibrated != nil` alone,
  covering both the windowed and the non-windowed-but-calibrated
  configurations. An estimate error here is non-fatal to the request:
  `Chat` still runs, `estimatedTokens` stays zero, and the later
...
- `api/agentloop.txt` is unaffected: `chatAttempt` is unexported, and
  `Result`, `Loop`, and `Options` keep their locked shape. Confirm
  this with `make api-update` producing no `agentloop` diff.
...
- The reference formula every case below computes against, stated
  ...
  `sample := actual / estimated`, then
  `next := factor * ((1-alpha) + alpha*sample)`, clamped to
  `[MinCorrectionFactor, MaxCorrectionFactor]`. A case that only
  asserts a "hand-computed expectation" with no stated formula is
  incomplete; every case below states or reuses this one.
- Converges toward `actual`, not toward `sqrt(raw*actual)`: builds a
  ...
  formula from the divisive drift: the divisive formula converges the
  scaled estimate near `sqrt(raw*actual)` instead, a different number
  whenever `raw != actual`, so this assertion fails against it.
- `Observe(estimated, actual)` pairs correctly when called out of
  estimate order: two `EstimateTokens` calls run back to back, then
  two `Observe` calls run with each call's own remembered estimate,
  in reverse order from how the estimates were produced. The
  resulting factor matches the reference formula applied in the order
  the `Observe` calls actually ran, proving the factor depends only on
  the arguments passed to each call, not on call order or on any
  earlier `EstimateTokens` call.
...
- `TestCalibratedConcurrentUse`, existing in this file: update its
  ...
  beyond "no panic": run N goroutines, each holding its own
  known `(estimated, actual)` pair distinct from every other
  goroutine's pair, call `Observe` on each pair concurrently on one
  shared `*Calibrated`, then assert the resulting `factor` equals the
  reference formula folded over every pair in some order —
  order-independence is not claimed; the reachable factor range across
  every valid application order of the reference formula is asserted
  instead, since EWMA is order-dependent by construction. This
  replaces a same-name test that only checked absence of a panic and
  a call count; the replacement must be able to fail against both the
  reverted, single-`lastEst` implementation and against a divisive
  formula.
- Delete or rewrite any existing case that asserts `Observe`
  no-ops "before any `EstimateTokens` call" as a described named rule;
  that rule no longer exists once `Observe` takes its own estimate
  argument.
...
- A deterministic-estimate `Completer` and estimator pair, where
  ...
  factor matches the `contextplan` reference formula (see this plan's
  Fix tests section) applied to the known `(estimated, actual)` pairs,
  instead of only asserting no panic.
- `TestRunConcurrentSharedLoopWithPlanning`, existing in
  `compaction_recovery_test.go`: add an assertion on the shared
  `Calibrated`'s resulting factor, computed with the same reference
  formula from the known, controlled `(estimated, actual)` pairs each
  goroutine's scripted `Completer` and estimator produce, proving the
  factor reflects correct pairing under concurrency, not merely the
  absence of a panic or a wrong call count.
...
- `api/contextplan.txt` changes through `make api-update`: the
  ...
  This is a breaking, deliberate change to a locked entry; commit the
  diff in the same change as the code, per AGENTS.md's API lock rule.
...
- `python3 scripts/check_plan.py`, `scripts/check_deps.py`, and
  ...
  neither package's import edges move.
...
- This fix lands with, or before, its own update to
  ...
  this change until that companion plan update and code land together.
