...
func TestRunLinearThreeSteps(t *testing.T) {
	...
	var order []string
	status, out, err := flow.Run(context.Background(), d, m, machine.InOut{}, recordingConfirm(&order))
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
	if status != s3 {
...
func TestRunDiamondTieBreak(t *testing.T) {
	...
	var order []string
	_, _, err = flow.Run(context.Background(), d, m, machine.InOut{}, recordingConfirm(&order))
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
