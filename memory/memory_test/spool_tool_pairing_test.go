// SpoolTool/ReadOutputTool pairing tests: a shared *memory.Spool wires
...
func TestSpoolToolNilSpool(t *testing.T) {
	wrapped, err := memory.SpoolTool("t", 10, nil, stringTool{name: "inner", result: "x"})
	if !errors.Is(err, memory.ErrNilSpool) {
...
func splitMoreMarker(page string) (body string, next int, more bool) {
	...
	var n int
	if _, err := fmt.Sscanf(page[idx:], memory.MoreMarker, &n); err != nil {
		return page, 0, false
	}
...
// tool with a ReadOutputTool over the same *memory.Spool: the tool
...
func TestSpoolToolReadOutputToolRoundTrip(t *testing.T) {
	...
	wrapped, err := memory.SpoolTool("big", 10, sp, stringTool{name: "inner", result: full})
	if err != nil {
		t.Fatalf("SpoolTool: %v", err)
	}
	readBack, err := memory.ReadOutputTool(sp, 200)
	if err != nil {
...
// one *memory.Spool share its grant budget: spooling the second
...
func TestSpoolToolSharedBudget(t *testing.T) {
	store := newFakeStore()
	sp, err := memory.NewSpool(store, 12)
	if err != nil {
		t.Fatalf("NewSpool: %v", err)
	}
	firstResult := strings.Repeat("a", 10)
	secondResult := strings.Repeat("b", 10)
	firstTool, err := memory.SpoolTool("first", 1, sp, stringTool{name: "inner-a", result: firstResult})
	if err != nil {
		t.Fatalf("SpoolTool(first): %v", err)
	}
	secondTool, err := memory.SpoolTool("second", 1, sp, stringTool{name: "inner-b", result: secondResult})
	if err != nil {
	...
	if _, err := sp.Load(ctx, "alice", firstRef); !errors.Is(err, memory.ErrUnknownGrantRef) {
		t.Errorf("Load(firstRef) err = %v, want ErrUnknownGrantRef: the shared budget should have evicted the older grant", err)
	}
