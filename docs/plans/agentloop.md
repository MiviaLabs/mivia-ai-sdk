...
- `Options.Validate`'s `Window` branch: after the existing
  `o.Summarizer == nil` check, a new check asserts
  `p, ok := o.Summarizer.(*plan.Summarizer); ok && p == nil`, still
  returning `ErrSummarizerRequired`.
...
- No API surface change: `Validate`'s signature and documented
  first-failure ordering are unchanged (the Window branch's internal
  check order is not part of the locked API).
