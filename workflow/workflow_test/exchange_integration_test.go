// Package workflow_test also holds the two-agent system-integration
...
// hop, wired around workflow.Agent's Run and AckWait, with no mock at
// any trust boundary.
...
// needs: Agent A's identity.Identity-backed workflow.Agent, the machine
...
func newExchangeFixture(t testing.TB, admitB bool) *exchangeFixture {
	...
	a, err := workflow.New(idA, card, plan)
	if err != nil {
...
// step runs.
func exchangeWait(t testing.TB, fx *exchangeFixture, captured *[]envelope.Message, refs *[]string) workflow.AckWait {
...
func TestExchangeSignedRequestConfirmedAck(t *testing.T) {
	...
	rec.subscribe(t, fx.bus, workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, flow.StepCompletedEvent, workflow.ThreadVerifiedEvent)
