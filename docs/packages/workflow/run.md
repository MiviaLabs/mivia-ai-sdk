# Package reference: workflow/run

The workflow/run package is the composition layer. It turns an agent, a
machine, and optional blocks into a runnable pipeline. One
`run.New` validates the transition matrix, the tool names, the
...
- `Agent.Plan()` — the step plan `workflow.New` bound.
...
- `ErrNoAgent` ("run: agent is required") — `New` returns it when
  `Options.Agent` is nil. Pinned by `run_test/options_test.go`.
- `ErrNoMachine` ("run: machine is required") — `New` returns it
  when `Options.Machine` is nil. Pinned by
  `run_test/options_test.go`.
- `ErrNoResolver` ("run: Wait or Tools is required") — `New`
  returns it when neither `Wait` nor `Tools` is set. Pinned by
  `run_test/options_test.go`.
- `ErrAmbiguousWait` ("run: Wait and Tools both set; set one") —
  `New` returns it when both `Wait` and `Tools` are set. Pinned by
  `run_test/options_test.go`.
- `ErrNoTools` ("run: Scope, Store, Ask, or Artifacts needs
  Tools") — `New` returns it when `Scope`, `Store`, `Ask`, or
  `Artifacts` is set without `Tools`. Pinned by
  `run_test/options_test.go`.
- `ErrNoRecipient` ("run: Ask needs AskTo") — `New` returns it
  when `Ask` is set with an empty `AskTo`. Pinned by
  `run_test/options_test.go`.
- `ErrResultNotText` ("run: tool result is not a string") — the
  built ack chain wraps it when a gated step's tool result is not a
  string. Pinned by `run_test/run_integration_test.go`.
- `ErrReceiverEmpty` ("run: Receiver signer is empty") — `New`
  returns it when the resolved receiver signer is empty, whether from
  `Options.Receiver` or from `Options.Agent`. Pinned by
  `run_test/options_test.go`.
- `ErrArtifactsInconsistent` ("run: artifacts state is
  inconsistent") — `Validate` wraps it for either invariant
  violation. Pinned by `run_test/artifacts_wire_test.go`.
- `ErrArgumentDecode` ("run: tool arguments failed to decode") —
...
`Runner.Run` also propagates two sentinels the `workflow` package
defines, through `errors.Is`: `workflow.ErrNoThread`, when `threadID` is
empty, and `workflow.ErrEscalated`, when a gated step's ack resolver
escalates to a human. Pinned by `run_test/options_test.go` and
`run_test/escalation_integration_test.go`.
...
- The built ack chain resolves a step's tool by `step.ID`. A suffixed
  repeat, which workflow.Run mints for a step confirmed twice, resolves
  ...
  `ErrResultNotText`.
- A chain error wrapping `workflow.ErrEscalated`, when `Ask` is set, routes
...
the older `_workflowcomposition` example writes by hand, compressed into one
`Options` literal. See [../../plans/workflow/run.md](../../plans/workflow/run.md).

## Cross-references

- [workflow.md](../workflow.md) — the composed agent `Runner` drives.
...
runner, err := run.New(run.Options{Agent: a, Machine: m,
