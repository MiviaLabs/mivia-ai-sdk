...
func agentOver(t *testing.T, d *runconfig.Definition) *workflow.Agent {
	...
	a, err := workflow.New(id, flow.Card{Name: "runner-test", Capabilities: []string{"cap"}}, d.Plan)
	if err != nil {
...
func innerRunner(t *testing.T) *run.Runner {
	...
	a, err := workflow.New(id, flow.Card{Name: "inner", Capabilities: []string{"cap"}}, plan)
	if err != nil {
		t.Fatalf("workflow.New: %v", err)
	}
	reg := tools.New()
