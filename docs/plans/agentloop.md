...
- `Definitions`, which builds `[]provider.ToolDefinition` from a
  `tools.Registry`, failing on a registered tool with no published
  schema and skipping a tool the scope denies.
...
- `type Options struct` — `Completer provider.Completer`,
  ...
  That field list is the phase-one shape. The current shape,
  `Options` plus an optional `Extensions` pointer, is the Options and
  Extensions split addendum below.
...
- `func Definitions(reg *tools.Registry, scope *tools.Scope) ([]provider.ToolDefinition, error)`
  — builds the offered set. A registered tool whose `tools.SchemaOf`
  check reports not-ok fails `Definitions` with `ErrNoSchema`,
  wrapped with the tool's registry name. A tool the scope denies is
  skipped silently: denial is policy filtering, not a mistake.
  `ErrNoSchemas` stays: a non-empty registry whose offered set ends
  up empty can now only mean the scope denied every tool. An empty
  Registry returns an empty set and no error. This contract
  supersedes the silent skip this bullet once described; see the
  Options and Extensions split addendum.
- Sentinel errors: `ErrNoCompleter`, `ErrNoTools`, `ErrMaxIterations`,
  `ErrUnrenderableResult`, `ErrCallsPerTurnExceeded`, `ErrNoSchema`,
  `ErrNoSchemas`, `ErrOverBudget`, `ErrTokenBudgetExceeded`.
...
- `spool/tool.go`'s `SpoolTool` doc comment changes its closing
...
- `spool/spool_test/tool_parity_test.go`'s header comment states the
  same consequence.
- `docs/plans/spool.md`'s Schema forwarding section drops its
...
- `e2e/e2e_test/anthropic_compaction_test.go`: regroup the triple
...
- `docs/plans/spool.md`: the Schema forwarding section, per the
...
- `spool/tool.go`'s skip-reliance sentences and `docs/plans/spool.md`'s
