...
def _probe_tt09_clean_outside_scope(tmp):
    """A deleted testdata/vectors/ file OUTSIDE the four scoped
    directories (envelope/, machine/, a2a/, mcp/) must not fire TT09:
    the _VECTOR_SCOPES restriction itself, not just the delete-vs-add
    distinction."""
    repo = _new_repo(tmp, "tt09_clean_outside_scope")
    vec_dir = repo / "ledger" / "testdata" / "vectors"
    vec_dir.mkdir(parents=True)
    (vec_dir / "valid_basic.json").write_text('{"a": 1}\n')
    commit_all(repo, "base")
    diffs = diff_after_change(repo, lambda r: (r / "ledger" / "testdata" / "vectors" / "valid_basic.json").unlink())
    findings = check_vector_deleted(diffs)
    if _has(findings, "TT09"):
        return [f"tt09 clean outside scope: unexpected TT09 for a path outside _VECTOR_SCOPES: {findings}"]
    return []


def _probe_tt10_clean_outside_scope(tmp):
    """A modified testdata/vectors/ file OUTSIDE the four scoped
    directories must not fire TT10."""
    repo = _new_repo(tmp, "tt10_clean_outside_scope")
    vec_dir = repo / "ledger" / "testdata" / "vectors"
    vec_dir.mkdir(parents=True)
    (vec_dir / "valid_basic.json").write_text('{"a": 1}\n')
    commit_all(repo, "base")
    diffs = diff_after_change(
        repo, lambda r: (r / "ledger" / "testdata" / "vectors" / "valid_basic.json").write_text('{"a": 2}\n')
    )
    findings = check_vector_modified(diffs)
    if _has(findings, "TT10"):
        return [f"tt10 clean outside scope: unexpected TT10 for a path outside _VECTOR_SCOPES: {findings}"]
    return []
...
def run_vector_probes(tmp) -> list:
    ...
    for fn in (
        ...
        _probe_tt09_clean_outside_scope,
        _probe_tt10_violation,
        _probe_tt10_clean_new_file,
        _probe_tt10_clean_outside_scope,
    ):
