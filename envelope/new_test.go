...
func TestLoadKeyFiles(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			id, err := envelope.Load("testdata/keyfile/" + tc.name)
			if tc.wantErr {
				if !errors.Is(err, envelope.ErrKeyFormat) {
...
func TestLoadMissingFile(t *testing.T) {
	_, err := envelope.Load("testdata/keyfile/does_not_exist")
	if err == nil {
...
func TestValidateFailures(t *testing.T) {
	mismatched, err := envelope.New()
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	other, err := envelope.New()
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	mismatched.PublicKey = other.PublicKey

	cases := map[string]envelope.Identity{
		"zero value":        {},
	...
	for name, id := range cases {
		t.Run(name, func(t *testing.T) {
			if err := id.Validate(); !errors.Is(err, envelope.ErrKeyInvalid) {
...
func TestSignValidatesFirst(t *testing.T) {
	var id envelope.Identity
	if _, err := id.Sign(envelope.Message{}); !errors.Is(err, envelope.ErrKeyInvalid) {
