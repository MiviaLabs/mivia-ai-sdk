// Package workflow_test also holds the whole-system composition test.
// One workflow.Run drives a graph with a panel wave, a retried step, a
...
func newSystemFixture(t testing.TB) *systemFixture {
	...
	if fx.a, err = workflow.New(fx.idA, card, systemPlan(t, seeded)); err != nil {
...
func systemWait(t testing.TB, fx *systemFixture, threadID string) workflow.AckWait {
...
// TestSystemCompositionRunsEveryShippedBlock drives one workflow.Run
...
func TestSystemCompositionRunsEveryShippedBlock(t *testing.T) {
	...
	rec.subscribe(t, fx.bus, workflow.MessageDeliveredEvent, workflow.MessageAckedEvent,
		flow.StepCompletedEvent, workflow.ThreadVerifiedEvent)
...
func assertSystemEvents(t *testing.T, rec *lifecycleRecorder) {
	...
	want := []events.Name{
		...
		workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, flow.StepCompletedEvent, // review
		workflow.MessageDeliveredEvent, workflow.MessageAckedEvent, flow.StepCompletedEvent, // recover
...
func TestSystemCompositionBudgetStopsTheRun(t *testing.T) {
	...
	t.Run("tight budget returns ErrOverBudget", func(t *testing.T) {
		...
		if !errors.Is(err, workflow.ErrOverBudget) {
			t.Fatalf("Run() error = %v, want errors.Is match for workflow.ErrOverBudget", err)
		}
		want := []events.Name{workflow.MessageDeliveredEvent}
		if !equalNames(rec.snapshot(), want) {
