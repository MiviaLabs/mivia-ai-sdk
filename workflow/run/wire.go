...
// threadID fails before any block runs, wrapping workflow.ErrNoThread. in
...
func (r *Runner) Run(ctx context.Context, threadID string, in machine.InOut) (machine.Status, machine.InOut, error) {
	if threadID == "" {
		return machine.Status(""), in, fmt.Errorf("run: %w", workflow.ErrNoThread)
	}
	if r.tracer != nil {
		var span *trace.Span
...
// NewAck. A suffixed message ID, which workflow.Run mints for a step
...
// wrapping workflow.ErrEscalated, when Ask is set, routes to one Ask
// round trip.
func (r *Runner) chain() workflow.AckWait {
	return func(ctx context.Context, msg envelope.Message) (envelope.Ack, error) {
		...
		if t, ok := r.tools.Get(name); ok {
			if _, ok := tools.SchemaOf(t); ok {
				...
				if derr != nil {
					return envelope.Ack{}, fmt.Errorf("run: step %q: %w: %w", msg.ID, ErrArgumentDecode, derr)
				}
				in = decoded
			}
		}
		...
		if err != nil {
			if r.ask != nil && errors.Is(err, workflow.ErrEscalated) {
		...
		if !ok {
			return envelope.Ack{}, fmt.Errorf("run: step %q: tool result not a string: %w", msg.ID, ErrResultNotText)
		}
		if r.store != nil {
...
func (r *Runner) askRoundTrip(ctx context.Context, msg envelope.Message) (envelope.Ack, error) {
	...
	if !answer.Approved {
		return envelope.Ack{}, fmt.Errorf("run: step %q declined by %q", msg.ID, r.askTo)
	}
...
func resolveGatedSteps(plan *flow.Definition, reg *tools.Registry) error {
	for _, id := range gatedStepIDs(plan) {
		if _, ok := reg.Get(id); !ok {
			return fmt.Errorf("run: step %q: %w", id, tools.ErrUnknownName)
		}
	}
	return nil
}
...
func DecodeArtifacts(data []byte) (*Artifacts, error) {
	...
	if err := json.Unmarshal(data, &w); err != nil {
		return nil, fmt.Errorf("run: artifacts decode: %w", err)
	}
...
func (a *Artifacts) checkInvariant() error {
	for step, runs := range a.runs {
		...
		if !ok || cur != last {
			return fmt.Errorf("run: step %q: %w: current value %q, last run %q", step, ErrArtifactsInconsistent, cur, last)
		}
	}
	for step := range a.values {
		if len(a.runs[step]) == 0 {
			return fmt.Errorf("run: step %q: %w: current value with no recorded run", step, ErrArtifactsInconsistent)
		}
	}
	return nil
}
